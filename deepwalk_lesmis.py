...
def generate_Graph(name):
    ...
    if name == "lesmis":
        ...
        nx.draw_networkx(G,pos, font_size = 20, node_size = 140,with_labels = True,width = 0.5)
...
def draw_network(G, with_labels = True, node_color_list=None):
    G = nx.read_gml("data/lesmis.gml")
    plt.figure(figsize=(36, 30))
    pos = nx.spring_layout(G,seed=0)
    nx.draw_networkx(G,pos,node_color = node_color_list, font_size = 20, node_size = 140,with_labels = True,width = 0.5)
    edge_labels = nx.get_edge_attributes(G, 'value')
    #nx.draw_networkx_edge_labels(G, pos, edge_labels = edge_labels,font_size = 10)
    plt.savefig("lesmis.png")
    plt.show()
...
"""
def draw_embedded_vector(Y,colorlist = [],node_annotation = None,ATTENTION_NODE_LIST = []):
    if len(colorlist) == 0:
    ...
    for i in range(len(Y)):
        ...
        if node_annotation != None:
            if node_annotation[i] in ATTENTION_NODE_LIST:
                ax.annotate(str(node_annotation[i]), (Y[i,0],Y[i,1]) ,color="red",fontsize=20)
            else:
                ax.annotate(str(node_annotation[i]), (Y[i,0],Y[i,1]),fontsize = 15)
        else:
            ax.annotate(str(i), (Y[i,0],Y[i,1]))

        if node_annotation[i] in ATTENTION_NODE_LIST:
            ax.scatter(Y[i,0],Y[i,1],s = 3000, c="red" ,alpha=0.2)
            ax.scatter(Y[i,0],Y[i,1],s = 70,c = colorlist[i])
...
"""
ベクトルデータからLoF（外れ値指標）を計算
"""
def culc_lof(vec,k=4):
    model = LocalOutlierFactor(n_neighbors=k)

    model.fit_predict(vec)
